...
using the following:
```
docker build --target=app -t alphabetcipher-app .
docker run alphabetcipher-app
```

To build and run the google tests, run the following:
```
docker build --target=tests -t alphabetcipher-tests .
docker run alphabetcipher-tests
```
